...
- [ ] **4.2 GHCR release cut** — tag `v0.1.0` on `main`; `.github/workflows/release.yml` (already shipped) publishes 5 services × 2 arches to `ghcr.io/flndrn-dev/briven-*`. → **Verify:** SBOM + provenance attestations visible on the package page; `docker pull ghcr.io/flndrn-dev/briven-api:0.1.0` works from a clean host.
