...
export interface ProjectLinkerProps {
  ...
  defaultBrivenProjectRef?: string
...
const ProjectLinker = ({
  ...
  const [brivenProjectRef, setBrivenProjectRef] = useState<string | undefined>(
    defaultBrivenProjectRef
  )

  const { data: selectedOrganization } = useSelectedOrganizationQuery()
  const { data: orgProjects, isPending: loadingBrivenProjects } = useOrgProjectsInfiniteQuery({
    slug,
  })
  const numProjects = orgProjects?.pages[0].pagination.count ?? 0

  useEffect(() => {
    if (defaultBrivenProjectRef !== undefined && brivenProjectRef === undefined)
      setBrivenProjectRef(defaultBrivenProjectRef)
  }, [defaultBrivenProjectRef, brivenProjectRef])
  ...
  const { data: selectedBrivenProject } = useProjectDetailQuery({ ref: brivenProjectRef })
  ...
  function onCreateConnections() {
    ...
    if (!selectedBrivenProject?.ref) return console.error('No Briven project ref set')
    ...
    _onCreateConnections({
      ...
      connection: {
        ...
        briven_project_ref: selectedBrivenProject?.ref,
        integration_id: '0',
        metadata: {
          ...projectDetails,
        },
      ...
      new: {
        ...
        project_ref: selectedBrivenProject.ref,
  ...
  const noBrivenProjects = numProjects === 0
  const noForeignProjects = foreignProjects.length === 0
  const missingEntity = noBrivenProjects ? 'Briven' : mode
  const oppositeMissingEntity = noBrivenProjects ? mode : 'Briven'
          ...
          <div className="flex justify-center gap-0 w-full relative">
            <Panel>
              <div className="bg-white shadow-sm border rounded-sm p-1 w-12 h-12 flex justify-center items-center">
                <img src={`${BASE_PATH}/img/briven-logo.svg`} alt="Briven" className="w-6" />
              </div>
              ...
              <OrganizationProjectSelector
                ...
                slug={slug}
                selectedRef={brivenProjectRef}
                onSelect={(project) => {
                  setBrivenProjectRef(project.ref)
                    ...
                    <div className={cn('w-full flex items-center justify-between')}>
                      <div className="flex items-center gap-x-2">
                        <div className="bg-white shadow-sm border rounded-sm p-1 w-6 h-6 flex justify-center items-center">
                          <img
                            src={`${BASE_PATH}/img/briven-logo.svg`}
                            alt="Briven"
                            className="w-4"
                          />
                        </div>
                      ...
                      {project.ref === brivenProjectRef && <Check size={16} />}
                    </div>
                  )
                }}
                renderTrigger={() => {
                  return (
                    <Button
                      type="default"
                      block
                      disabled={defaultBrivenProjectRef !== undefined || loadingBrivenProjects}
                      loading={loadingBrivenProjects}
                      className="justify-between h-[34px]"
                      iconRight={
                        defaultBrivenProjectRef === undefined ? (
                    ...
                    >
                      <div className="flex items-center gap-x-2">
                        ...
                        {selectedBrivenProject
                          ? selectedBrivenProject.name
                          : 'Choose Briven Project'}
                      </div>
                    </Button>
                  )
                }}
