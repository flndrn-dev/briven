...
export const EdgeFunctionDetails = () => {
  ...
  const invocationTabs = useMemo(() => {
    ...
    return INVOCATION_TABS.filter((tab) => tab.id === 'curl' || tab.id === 'briven-js')
              ...
              <div className="rounded-sm border bg-surface-100 px-6 py-4 drop-shadow-xs">
                <div className="space-y-6">
                  <CommandRender
                    commands={[
                      {
                        command: `briven functions download ${selectedFunction?.slug}`,
                        description: 'Download the function to your local machine',
                        jsx: () => (
                          <>
                            <span className="text-brand">briven</span> functions download{' '}
