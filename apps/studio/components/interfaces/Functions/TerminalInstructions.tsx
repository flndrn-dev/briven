...
>(({ closable = false, removeBorder = false, ...props }, ref) => {
  ...
  const commands: Commands[] = [
    {
      command: 'briven functions new hello-world',
      description: ' creates a function stub at ./functions/hello-world/index.ts',
      jsx: () => {
        return (
          <>
            <span className="text-brand-600">briven</span> functions new hello-world
          </>
        )
      },
    ...
    {
      command: `briven functions deploy hello-world --project-ref ${projectRef}`,
      description: 'Deploys function at ./functions/hello-world/index.ts',
      jsx: () => {
        return (
          <>
            <span className="text-brand-600">briven</span> functions deploy hello-world
    ...
    {
      command: `curl -L -X POST 'https://${projectRef}.briven.${restUrlTld}/functions/v1/hello-world' -H 'Authorization: Bearer ${apiKey}'${anonKey?.type === 'publishable' ? ` -H 'apikey: ${apiKey}'` : ''} --data '{"name":"Functions"}'`,
            ...
            <div className="flex gap-2">
              ...
              <Button asChild type="default" icon={<ExternalLink />}>
                <a
                  target="_blank"
                  rel="noreferrer"
                  href="https://github.com/briven/briven/tree/master/examples/edge-functions/briven/functions"
                >
                  Examples
                </a>
              </Button>
