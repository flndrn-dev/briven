...
const brivenServiceKey = process.env.NEXT_PUBLIC_BRIVEN_ANON_KEY
...
export default async function handler(req: NextRequest) {
  ...
  if (!brivenUrl) {
    return new Response(
      JSON.stringify({
        error:
          'No NEXT_PUBLIC_BRIVEN_URL set. Create this environment variable to use AI features.',
      }),
      {
        status: 500,
        headers: { 'Content-Type': 'application/json' },
  ...
  if (!brivenServiceKey) {
    return new Response(
      JSON.stringify({
        error:
          'No NEXT_PUBLIC_BRIVEN_ANON_KEY set. Create this environment variable to use AI features.',
      }),
      {
        status: 500,
        headers: { 'Content-Type': 'application/json' },
