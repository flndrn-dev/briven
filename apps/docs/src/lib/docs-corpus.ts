...
export const DOCS_CORPUS: readonly DocsCorpusEntry[] = [
  ...
  },
  {
    ...
    summary:
      'step-by-step setup: install the cli, run briven init, define a schema, write a function, deploy, invoke it from the browser. also surfaces the dashboard-only path (no cli) for users who want to try the platform first.',
    keywords: [
      'install',
      'setup',
      'init',
  ...
  },
  {
    ...
    title: 'cli reference',
    summary:
      'every briven cli command — init, login, link, deploy, dev, logs, db shell, env, invoke, projects, export, import, doctor, whoami. one section per command with usage, flags, and exit codes.',
    keywords: [
      'cli',
      'briven init',
      'briven deploy',
  ...
  },
  {
    ...
    summary:
      'starter templates available via `briven init --template`. lists each template with its tech-stack pairing and the briven features it shows off.',
    keywords: ['templates', 'starter', 'briven init', 'scaffold', 'boilerplate'],
  },
  {
    slug: '/migration',
  ...
  {
    slug: '/auth',
    title: 'auth · @briven/auth sign-in sdk (email/password, magic link, OTP, OAuth)',
    summary:
      'drop-in authentication for a briven project via @briven/auth (public npm). createBrivenAuth({ projectId, publicKey }) with the browser-safe pk_briven_auth_ key from the dashboard Auth → API keys panel. React bindings from @briven/auth/react: BrivenAuthProvider, useSession, useUser, and the prebuilt BrivenSignIn component. flows: email + password, magic link (its own passwordless flow, not an OAuth provider), email OTP, and OAuth social (google, github, discord, microsoft, konnos). social login needs the provider client id + secret configured; magic-link/OTP need platform email/SMTP. pk key is browser-safe, brk_ server keys are not.',
    keywords: [
      'auth',
      'authentication',
      'sign in',
