...
export async function provisionProjectRole(projectId: string): Promise<void> {
  ...
  // (default database), not inside the project database. It is not idempotent
  // via IF NOT EXISTS, so guard with a DO block that checks pg_roles. The role
  // name is generated by us (roleNameFor sanitizes the project id), so it is
  // safe to inline.
  await admin.query(`
    DO $$
    BEGIN
