 ...
 *
 * These tables live in each project's data-plane schema (`proj_<projectId>`),
 ...
 * single transactional migration that emits the CREATE TABLE + CREATE
 * EXTENSION citext statements derived from these models, then records the
 * applied migration in `_briven_migrations` like every other schema apply.
 */
 ...
 * email within the tenant. `email` uses `citext` at the postgres level —
 * Drizzle treats it as `text` for query typing; the CREATE TABLE step
 * (in the provisioning emitter) ensures the underlying column is citext
 * after the `CREATE EXTENSION IF NOT EXISTS citext` runs.
 */
 ...
 *
 * `metadata` is a small jsonb blob carrying context that doesn't deserve
...
/** OAuth provider ids known to briven auth v1. Apple is intentionally
 *  omitted — see BUILD_PLAN.md "Decisions locked" Q8 (Apple Sign-In is
 *  out of scope for v1: no Apple developer account). */
