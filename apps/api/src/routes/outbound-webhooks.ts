...
outboundWebhooksRouter.use('/v1/projects/:id/outbound-webhooks', requireProjectAuth());
outboundWebhooksRouter.use('/v1/projects/:id/outbound-webhooks/*', requireProjectAuth());
...
outboundWebhooksRouter.patch(
  ...
  async (c) => {
    ...
    if (!parsed.success) return c.json(validationResponse(parsed.error.issues), 400);
    try {
      const subscriber = await updateSubscriber(subscriberId, projectId, parsed.data);
      await audit({
        actorId,
        projectId,
...
outboundWebhooksRouter.post(
  ...
  async (c) => {
    ...
    const actorId = c.get('user')?.id ?? null;
    const subscriberId = c.req.param('subscriberId');
    const result = await rotateSubscriberSecret(subscriberId, projectId);
    await audit({
      actorId,
      projectId,
...
outboundWebhooksRouter.delete(
  ...
  async (c) => {
    ...
    const actorId = c.get('user')?.id ?? null;
    const subscriberId = c.req.param('subscriberId');
    await deleteSubscriber(subscriberId, projectId);
    await audit({
      actorId,
      projectId,
  ...
  async (c) => {
    ...
    const status =
      statusParam && (DELIVERY_STATUS_VALUES as readonly string[]).includes(statusParam)
        ...
        : undefined;
    const deliveries = await listOutboundDeliveries(subscriberId, projectId, {
      limit: 100,
      status,
    });
