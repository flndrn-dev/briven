...
- **Auth** - against [briven/auth/README.md](https://github.com/briven/auth/blob/master/README.md)
- **PostgREST** - against [postgrest.org/en/stable](https://docs.postgrest.org/en/stable/references/configuration.html)
- **Realtime** - against [briven/realtime/ENVS.md](https://github.com/briven/realtime/blob/main/ENVS.md)
- **Storage** - against the Briven docs [YAML spec](https://github.com/briven/briven/blob/master/apps/docs/spec/storage_v0_config.yaml) for the variables it covers; otherwise from code reads
- **Analytics (Logflare)** - against the Briven docs [YAML spec](https://github.com/briven/briven/blob/master/apps/docs/spec/analytics_v0_config.yaml) and [docs.logflare.app/self-hosting](https://docs.logflare.app/self-hosting/) for the rest, with [logflare/config/runtime.exs](https://github.com/Logflare/logflare/blob/main/config/runtime.exs) and [logflare/config/config.exs](https://github.com/Logflare/logflare/blob/main/config/config.exs) used as tiebreakers
- **Supavisor** - against [briven/supavisor/docs/configuration/env.md](https://github.com/briven/supavisor/blob/main/docs/configuration/env.md)
...
- **The CLI does not run Supavisor** as part of `briven start`, so every Supavisor variable's `Set by` is either `Self-hosted` or blank - never `Both` or `CLI`.
...
|---|---|---|
| Studio (Dashboard) | `briven/studio` | [briven/briven/apps/studio](https://github.com/briven/briven/tree/master/apps/studio) |
| Auth | `briven/gotrue` | [briven/auth](https://github.com/briven/auth) |
| PostgREST | `postgrest/postgrest` | [PostgREST/postgrest](https://github.com/PostgREST/postgrest) |
| Realtime | `briven/realtime` | [briven/realtime](https://github.com/briven/realtime) |
| Storage | `briven/storage-api` | [briven/storage](https://github.com/briven/storage) |
| Edge Functions | `briven/edge-runtime` | [briven/edge-runtime](https://github.com/briven/edge-runtime) |
| Analytics | `logflare/logflare` | [logflare/logflare](https://github.com/logflare/logflare) |
| Postgres | `briven/postgres` | [briven/postgres](https://github.com/briven/postgres) |
| Supavisor (Pooler) | `briven/supavisor` | [briven/supavisor](https://github.com/briven/supavisor) |
...
- [Postgres (briven/postgres)](#postgres)
- [Supavisor](#supavisor)
...
| `POSTGRES_USER_READ_ONLY` | string | | Postgres role used for read-only queries from the SQL editor. | Default: `briven_read_only_user`. Only takes effect if you've manually created the role per the "remove superuser access" guide. |
| `POSTGRES_USER_READ_WRITE` | string | Both | Postgres role used for read/write queries from the SQL editor. | Default: `briven_admin`. Commented out in default compose. See "remove superuser access" guide. |
| `STUDIO_PG_META_URL` | URL | Both | URL of the `postgres-meta` service used for schema introspection. | E.g. `http://meta:8080`. Required. |
| `BRIVEN_PUBLIC_URL` | URL | Both | Public URL of the Briven stack (Kong gateway) as seen by end users. | Used to construct REST API URLs and connection strings shown in the dashboard. |
| `BRIVEN_URL` | URL | Both | Internal URL Studio uses to reach Kong from inside the Docker network. | E.g. `http://kong:8000`. |
...
| `BRIVEN_ANON_KEY` | string | Both | Anon API key surfaced in the Project API Settings page and used by in-dashboard clients. | Mapped from `ANON_KEY`. Supports `_FILE` suffix for Docker secrets. |
| `BRIVEN_SERVICE_KEY` | string | Both | Service-role API key surfaced in the Project API Settings page. | Mapped from `SERVICE_ROLE_KEY`. Supports `_FILE` suffix for Docker secrets. Keep secret. |
...
| `LOGFLARE_PUBLIC_ACCESS_TOKEN` | string | Self-hosted | Public API token used by the analytics (briven/logflare) container for ingestion. | Not read by Studio code (despite being in `apps/studio/turbo.jsonc`). Passed through the `studio` service env in `docker-compose.yml` for parity only. |
...
| `NEXT_ANALYTICS_BACKEND_PROVIDER` | enum | Both | Historically intended to select the analytics container's backend (`postgres` or `bigquery`). | No-op today: not read by Studio code, and the `analytics` (briven/logflare) container chooses its backend via `POSTGRES_BACKEND_URL` / `LOGFLARE_FEATURE_FLAG_OVERRIDE` instead. Safe to ignore. |
...
| Variable | Type | Set by | Description | Notes |
|---|---|---|---|---|
| `CURRENT_CLI_VERSION` | string | CLI | Version string set when Studio is started by the Briven CLI. | Renames the default project to "Briven Studio (CLI)" when set. Exposed to client via Next.js passthrough. |
...
> Auth's upstream `README.md` documents many of these variables with additional prose context - operator/Netlify history, default email-template bodies, OAuth provider examples, glob-matching syntax. The rows below stay reference-style; for prose backstory and template defaults, see [briven/auth/README.md](https://github.com/briven/auth/blob/master/README.md).
...
| `GOTRUE_JWT_ADMIN_ROLES` | string (CSV) | Both | Comma-separated roles treated as admin. | Default: `service_role,briven_admin` |
...
| `GOTRUE_PASSWORD_HIBP_USER_AGENT` | string |  | User-Agent sent to the HIBP API. | Default: `https://github.com/briven/gotrue` |
...
> Realtime's upstream env-var reference is at [briven/realtime ENVS.md](https://github.com/briven/realtime/blob/main/ENVS.md).
...
| `DB_USER` | string | Both | Postgres user. | Default: `briven_admin`. |
...
| `SLOT_NAME_SUFFIX` | string | CLI | Suffix appended to the default replication slot name `briven_realtime_replication_slot`. | Allowed: lowercase letters, numbers, underscore. Combined name must be 64 characters or fewer. |
...
| Variable | Type | Set by | Description | Notes |
|---|---|---|---|---|
| `DATABASE_APPLICATION_NAME` | string |  | Postgres `application_name` for the API connection pool. | Default: `Briven Storage API ${VERSION}` |
...
| `EXT_AI_CACHE_DIR` | path | | Directory used to cache ONNX model files downloaded by `Briven.ai`. | Defaults to OS cache dir |
...
| `OMP_NUM_THREADS` | integer (count) | | Number of intra-op threads for the ONNX runtime used by `Briven.ai`. | Default: `1` |
...
| `BRIVEN_ANON_KEY` | JWT | Both | Public ("anonymous") Briven API key. Injected for user functions to call the public API. | Injected for user functions |
| `BRIVEN_DB_URL` | URL | Both | Postgres connection string. Injected for user functions that connect directly to Postgres. | Injected for user functions |
| `BRIVEN_INTERNAL_FUNCTIONS_CONFIG` | JSON | CLI | JSON map of per-function options (e.g. `verify_jwt`, `import_map_path`) consumed by the CLI's bundled main service. | Set by CLI; consumed by main service |
| `BRIVEN_INTERNAL_HOST_PORT` | integer | CLI | Local API port the CLI's bundled main service forwards requests to. | Set by CLI |
| `BRIVEN_INTERNAL_JWT_SECRET` | JWT | CLI | HS256 secret used by the CLI's bundled main service to verify JWTs from the local stack. | Set by CLI |
| `BRIVEN_INTERNAL_PUBLISHABLE_KEY` | string | CLI | Opaque API key (publishable) used internally by the CLI's bundled main service. | Set by CLI |
| `BRIVEN_INTERNAL_SECRET_KEY` | string | CLI | Opaque API key (secret) used internally by the CLI's bundled main service. | Set by CLI |
| `BRIVEN_JWKS` | JWKS | CLI | JSON Web Key Set (asymmetric + legacy symmetric) used by the bundled main service to verify user JWTs. | Self-hosted can derive this from `BRIVEN_URL`'s `/auth/v1/.well-known/jwks.json`. |
| `BRIVEN_PUBLIC_URL` | URL | Self-hosted | External/public URL of the Briven project. Injected for user functions. | Injected for user functions |
| `BRIVEN_PUBLISHABLE_KEYS` | JSON | Self-hosted | JSON map of opaque publishable API keys (new asymmetric-key format). | Injected for user functions |
| `BRIVEN_SECRET_KEYS` | JSON | Self-hosted | JSON map of opaque secret API keys (new asymmetric-key format). Never expose to client code. | Injected for user functions |
| `BRIVEN_SERVICE_ROLE_KEY` | JWT | Both | `service_role` API key (full database access). Injected for user functions for privileged calls. | Injected for user functions |
| `BRIVEN_URL` | URL | Both | Internal Briven API URL (Kong gateway hostname in self-hosted setups). Injected for user functions. | Injected for user functions |
...
| `VERIFY_JWT` | boolean | Self-hosted | If `true`, the bundled main service rejects requests whose JWT does not verify against `JWT_SECRET`/`BRIVEN_JWKS`. Applies to all functions. | Read by `docker/volumes/functions/main/index.ts`; supplied via `FUNCTIONS_VERIFY_JWT` in `.env.example` |

---

## Analytics

> The `analytics` container runs [logflare/logflare](github.com/Logflare/logflare), an Elixir/Phoenix application. Almost all runtime env reads live in [config/runtime.exs](https://github.com/Logflare/logflare/blob/main/config/runtime.exs). Self-hosted Briven runs it in single-tenant Briven mode with the Postgres backend; BigQuery support is available but commented out in `docker-compose.yml`. The container is the consumer of `LOGFLARE_PUBLIC_ACCESS_TOKEN`/`LOGFLARE_PRIVATE_ACCESS_TOKEN`.

> **Heads-up - always-on admin UI:** Logflare's admin pages under `/admin/*` (sources, accounts, cluster view) **are reachable by default**. `LOGFLARE_BRIVEN_MODE=true` provisions an auto-admin user, and the `/admin/*` routes are gated by an auth pipeline rather than an env var - there is no flag to disable them.
>
...
| `LOGFLARE_BRIVEN_MODE` | boolean | Both | Enable the Briven preset: auto-creates the default source, wires the `analytics` container to the Briven stack. | Default: `false`. Self-hosted: `true` |
...
> These configure Logflare's own metadata Postgres connection (tenants, sources, endpoints). Self-hosted points them at the shared `briven-db` container, schema `_analytics`.
...
| `DB_DATABASE` | string | Both | Database name for Logflare's metadata Postgres. | Self-hosted: `_briven` |
| `DB_SCHEMA` | string | Both | Postgres schema used for Logflare metadata tables (set as `search_path`). | Self-hosted: `_analytics` |
| `DB_USERNAME` | string | Both | Postgres user for Logflare's metadata connection. | Self-hosted: `briven_admin` |
...
> The `db` container runs the `briven/postgres` image, a fork of the official `postgres` image that adds Briven-specific extensions (`pgsodium`, `pg_graphql`, `pgjwt`, etc.), default roles, and seed migrations. Most variables are inherited from the upstream `postgres` image and read by its `docker-entrypoint.sh` on first boot (initdb). A few are added by the Briven fork or by init SQL that the self-hosted compose mounts into `/docker-entrypoint-initdb.d/init-scripts/`.
...
| `POSTGRES_USER` | string | CLI | Username for the initial superuser. The Briven image overrides this. | Default: `briven_admin` (Briven image) |
| `POSTGRES_DB` | string | Both | Name of the first database to create. | Default: `postgres` |
| `POSTGRES_HOST` | string | Both | Unix socket directory or hostname Postgres listens on. The Briven image hardcodes the socket path. | Default: `/var/run/postgresql` (Briven image) |
| `POSTGRES_PORT` | integer | Self-hosted | TCP port Postgres listens on (Briven migration scripts also read this). | Default: `5432` |
...
### libpq client variables (read by Briven migration scripts on init)

> The Briven image's `migrations/db/migrate.sh` runs at first boot and reads the standard libpq env vars rather than the `POSTGRES_*` ones. The compose file sets both so the entrypoint and the migration runner both work.
...
### Briven init SQL (mounted by docker-compose)

> These are consumed by SQL scripts the self-hosted compose mounts into `/docker-entrypoint-initdb.d/init-scripts/`. They are *not* read by the `briven/postgres` image itself - they are read by init SQL under `docker/volumes/db/` (the orchestration layer).
...
> Supavisor's upstream env-var reference is at [briven/supavisor/docs/configuration/env.md](https://github.com/briven/supavisor/blob/main/docs/configuration/env.md).
...
| `JWT_CLAIM_VALIDATORS` | JSON | | JSON object of additional JWT claims to validate (e.g. `{"iss":"briven"}`). | Default: `{}` |
